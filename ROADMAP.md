...
P5 局部进展：已实现 Run 结果契约、单次执行、流式运行作用域、主动取消及最终结果查询；运行过程统一由 AgentEvent 表达。Runtime 显式流关闭的双重故障交付已完成，全部 152 项离线测试通过；复核发现 Loop 内部模型流关闭仍可能覆盖原模型异常，P5 保持未完成。见 docs/p5-runtime-cleanup-failures.md。
